...
export const createOrder = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    try {
      ...
      if (!customerInfo?.shippingAddress?.email || 
      ...
      const { order, orderAmounts, totalAmount } = await createOrderInternal(customerInfo, cartItems, products)
...
async function createOrderInternal(customerInfo: CustomerInfo, cartItems: CartItem[], products: ProductWithVariations[]) {
  const db = DB() as any
